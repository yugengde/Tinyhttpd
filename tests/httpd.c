...
void accept_request_v2(void *arg){
    int n;
    ...
    while(1){
        ...
        unsigned long epoch = cJSON_GetNumberValue(cJSON_GetObjectItem(data, "epoch"));
        printf("\n\n=======================\n");
        printf("ecpch = %lu\n", epoch);
...
void accept_request(void *arg)
{
    ...
    if (strcasecmp(method, "GET") && strcasecmp(method, "POST"))  // !GET && !POST
    {
    ...
    {
        query_string = url;
        while ((*query_string != '?') && (*query_string != '\0'))  // 获取query_string的开始的地址(指针)
            query_string++;
        if (*query_string == '?')
        {
            cgi = 1;
    ...
    {
        if ((st.st_mode & S_IFMT) == S_IFDIR)  // https://blog.csdn.net/simmerlee/article/details/8281399
            strcat(path, "/index.html");
        if ((st.st_mode & S_IXUSR) ||
                (st.st_mode & S_IXGRP) ||
                (st.st_mode & S_IXOTH)    )  // st_mode: 文件的类型和访问权限
            cgi = 1;
        if (!cgi)
...
int main(void)
{
    ...
    */
    ...
    while (1)
    {
        ...
        if (pthread_create(&newthread , NULL, (void *)accept_request_v2, (void *)(intptr_t)client_sock) != 0)
